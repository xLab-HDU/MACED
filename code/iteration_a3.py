...
parser.add_argument('--path', default="result/bsds/round2/a1+a2/epoch-9-training-record/epoch-9-checkpoint.pth", type=str)
...
def adjust_weights_based_on_mask(weights, loss_mask,ep_mask):
    ...
    weights[loss_mask == 3] *= 3  # 减小权重
    weights[loss_mask == 2] *= 0.5 # 减小权重
    weights[loss_mask == 1] *= 0  # 减小权重
    weights[ep_mask == 1] *= 10
...
def cross_entropy_loss_RCF_update(prediction, labelef, std, ada, loss_mask, ep_mask):  # 二元交叉熵损失
    ...
    bce_loss = F.binary_cross_entropy(
        prediction, labelef.float(), weight=adjusted_weights.detach(), reduction='none')
...
def iter(mask, len_epoch, save_dir, bi_dict,loss_mask,ep_mask,label,threshold=0.6,last_threshold=0.7):
    ...
    mask_binary = torch.full_like(normalized_mask, 0.75)
...
#################################
        ...
        if epoch >= iter_begin_epoch:
            ...
            for j in range(args.batch_size):
                ...
                mask_condition = (ep_mask[j, 0, :, :] >= 0.247 - tolerance) & (ep_mask[j, 0, :, :] <= 0.247 + tolerance)
...
if __name__ == '__main__':
    ...
    time.sleep(9 * 60 * 60)  # 3 小时 = 3 * 60 分钟 * 60 秒
    main()
