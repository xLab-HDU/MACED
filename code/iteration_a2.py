...
parser.add_argument('--gpu', default='1', type=str, help='GPU ID(s), e.g., "0,1" for multi-GPU')
...
parser.add_argument('--path', default="result/bsds/round2/a1/epoch-9-training-record/epoch-9-checkpoint.pth", type=str)
...
def adjust_weights_based_on_mask(weights, loss_mask, ep_mask, tol=1e-3):
    """根据 cv-mask 和 ep-mask 调整像素权重

    weights   : new_mask = mask * exp(std*ada)
    loss_mask : 1–5 置信度
    ep_mask   : 1 / 0.498 / 0
    """
    ...
    # --------- 原来的 cv 置信度倍率 ----------
    weights[loss_mask == 4] *= 5
    weights[loss_mask == 3] *= 4
    weights[loss_mask == 2] *= 0.5
    weights[loss_mask == 1] *= 0

    # --------- 原来的 ep 可信像素加权 ----------
    weights[ep_mask == 1] *= 10          # 可信正／可信负

    # --------- 新增：忽略“不确定”像素 ----------
    uncertain = torch.isclose(ep_mask, torch.tensor(0.498, device=ep_mask.device),
                              atol=tol)
    weights[uncertain] = 0               # 直接置零 → BCE 不计算梯度
...
#     cv_mask_norm = loss_mask / 5.0
#     cv_mask_norm = cv_mask_norm.to(mask.device)
...
#     #
#     # return cost, mask
#     new_mask = mask * torch.exp(std * ada) * cv_mask_norm
#
#     bce_loss = F.binary_cross_entropy(
#         prediction, labelef.float(), weight=new_mask.detach(), reduction='none')
#
#     loss_per_pixel = torch.exp(-std) * bce_loss + 0.5 * std
#
#     # 7. 最终损失 (这里直接求和，你可以根据需要调整为 mean)
...
def save_mask_as_png(mask_binary, output_dir, bi_dict):
    ...
    for idx in range(mask_binary.shape[0]):
        mask = mask_binary[idx].detach().cpu().numpy() * 255
...
def main():
    ...
    cv_mask_total = torch.zeros_like(mask, device=device)
    ep_mask_total = torch.zeros_like(mask, device=device)
    ...
    for epoch in range(args.start_epoch, args.maxepoch):
        ...
        train(train_loader, model, optimizer, epoch,    #在一个 epoch 内使用训练数据集对模型进行训练
          save_dir=join(TMP_DIR, 'epoch-%d-training-record' % epoch), running_predictions=running_predictions, bi_dict=bi_dict, iter_begin_epoch=iter_begin_epoch,labels = labels,cv_mask_total = cv_mask_total,ep_mask_total=ep_mask_total)
...
def iter(mask, len_epoch, save_dir, bi_dict,loss_mask,ep_mask,label,threshold=0.7,last_threshold=0.8):
    ...
    # 2. Get all images from folder (adjust path to your images)
    ...
    mask_binary = torch.full_like(normalized_mask, 0.25)
    ...
    # mask_binary[(normalized_mask >= threshold) &(normalized_mask <= last_threshold) & (loss_mask >= 4) & surrounding_has_one] = 0.96
    # 将小于 lower_threshold 的部分变成 0.9_mask_a1+a2.9_mask_a1.1,代表是a2的
    # mask_binary[normalized_mask < (1.0 - threshold)] = 0.1  原来是这个
    mask_binary[(normalized_mask < (1.0 - threshold)) &(loss_mask >= 3) &(label==0)] = 0


    #condition = (ep_mask > 0) &(ep_mask < 0.02) & (mask_binary != 0)

    # 2. 将 `mask_binary` 的值赋给满足条件的 `ep_mask`
    #ep_mask[condition] = mask_binary[condition]
...
def save_tensor_as_txt(tensor, save_path, name):
    filename = os.path.splitext(name)[0] + '.txt'  # Convert to .txt extension
    filepath = join(save_path, filename)
    with open(filepath, 'w') as f:
        # Write the tensor data to the file
        for i in range(tensor.shape[0]):  # For each slice in the batch
            f.write(f"Slice {i}:\n")
            np.savetxt(f, tensor[i].cpu().numpy(), fmt='%.4f')


def train(train_loader, model, optimizer, epoch, save_dir, running_predictions, bi_dict, iter_begin_epoch,labels,cv_mask_total,ep_mask_total):
    global cv_mask
    ...
    ###########
    ep_mask_images = get_all_ep_mask_image_paths(r'E:\UAED\6.UAED\6.UAED\result\bsds\round2\a1\mask_a11')  # 自定义函数返回所有 ep_mask 图片路径
...
#################################
        ...
        if epoch >= iter_begin_epoch:
            ...
            for j in range(args.batch_size):
                ...
                cv_mask_total[idx, :label.shape[2], :label.shape[3]] = loss_mask[j, 0, :, :]
                labels[idx, :label.shape[2], :label.shape[3]] = label[j, 0, :, :]  # 1600*256*160的label
                running_predictions[idx, :_weight, :_height][mask_condition] += outputs[j, 0, :, :][mask_condition]  # 1600*256*160的累计预测值
                ep_mask_total[idx, :label.shape[2], :label.shape[3]] = ep_mask[j, 0, :, :]
    if epoch == args.maxepoch - 1:
        save_iter_dir =r'E:\UAED\6.UAED\6.UAED\result\bsds\round2\a1+a2'
        iter(running_predictions, args.maxepoch - 0, save_dir=save_iter_dir, bi_dict=bi_dict,
              loss_mask=cv_mask_total, label=labels,ep_mask=ep_mask_total)
...
if __name__ == '__main__':
    ...
    # time.sleep(5 * 60 * 60)  # 3 小时 = 3 * 60 分钟 * 60 秒
    main()
